...
const { Config, User } = require('../database'); // ← اصلاح import
const { v4: uuidv4 } = require('uuid'); // ← اصلاح import
const crypto = require('../core/crypto');
const logger = require('../core/logger');

const VLESSGenerator = require('../generator/vless'); // ← اصلاح اسم
...
router.get('/', async (req, res) => {
    try {
        const configs = await Config.findAll({
            ...
            order: [['createdAt', 'DESC']] // ← اصلاح createdAt
        });
        res.json(configs);
    } catch (error) {
...
router.post('/', async (req, res) => {
    try {
        const { type, name, server, port, userId, ...rest } = req.body; // ← اصلاح destructuring
